            ...
            foreach($quto as $k=>$v){
                ...
                if($v['automatic']==0){
                    ...
                    /*if (in_array($v['quota_id'],C('QUARTER_QUOTA_ID')) && $v['month']>201903){
                        ...
                        //工单及时性 (工单)
                        //176=>工单及时性-PHP程序员
                        if (in_array($v['quota_id'],array(130,136,148,150,186,231,176))){
                            //及时率
                            $uids                   = array($user);
                        ...
                        //季度顾客满意度
                        if (in_array($v['quota_id'],array(126,226,249))){
                        ...
                        if (in_array($v['quota_id'],array(92,129,155,162,168,193,206,214,212,218,219,182))){
                            ...
                            $type                   = M('satisfaction_user')->where(array('account_id'=>$v['user_id']))->getField('type');
                            $pin                    = $type ? $type : 1;
                            $url                    = U('Inspect/satisfaction',array('uname'=>$uname,'kpiTime'=>$v['start_date'].','.$v['end_date'],'pin'=>$pin));
                        }
                        ...
                        //233=>工作负荷率
                        //175=>工作负荷率-PHP程序员
                        if (in_array($v['quota_id'],array(175,233))){
                        ...
                        if (in_array($v['quota_id'],array(234,242,238,247))){
                            ...
                            $target                 = $v['target']; //目标
                            $complete               = $data['sum_profit']; //累计完成毛利
                        ...
                        //250  合同签订率(业务部门经理)
                        if ($v['quota_id']==250){
                                $department             = M()->table('__ACCOUNT__ as a')->join('__SALARY_DEPARTMENT__ as d on d.id = a.departmentid','left')->field('d.id , d.department')->where(array('a.id'=>$v['user_id']))->find();
                                $yearMonth              = substr($v['month'],0,6);
                        ...
                        if ($v['quota_id']==254){
                            ...
                            $mod                        = D('Manage');
                            // 季度经营报表
                            $quart                      = quarter_month1($monon);//获取季度月份
...
function get_cost_save_type($month){
...
function get_holidays(){
    $holiday=[
        '2020-01-01','2020-01-04','2020-01-05','2020-01-11','2020-01-12','2020-01-18','2020-01-19','2020-01-20','2020-01-21','2020-01-22','2020-01-23','2020-01-24','2020-01-25','2020-01-26','2020-01-27','2020-01-28','2020-01-29','2020-01-30',
        ...
        '2020-04-04','2020-04-05','2020-04-06','2020-04-11','2020-04-12','2020-04-18','2020-04-19','2020-04-25','2020-04-26',
 ...
 * @return array
 */
function get_year_begin_to_quarter_end_cycle($year,$quarter){
    $data                           = array();
    $data['begin_time']             = strtotime(($year-1).'1226') ;
    switch ($quarter){
        case 1:
