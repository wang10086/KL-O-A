            ...
            foreach($quto as $k=>$v){
                ...
                if($v['automatic']==0){
                    ...
                    /*if (in_array($v['quota_id'],C('QUARTER_QUOTA_ID')) && $v['month']>201903){
                        ...
                        if (in_array($v['quota_id'],array(129,155,168,193,206,214,212,218,219,182))){
                            $data                   = get_company_satisfaction($v);

                            //$complete               = get_kpi_satis($v,$data);
                            $complete               = $data['sum_average'];
                            $uname                  = M('account')->where(array('id'=>$v['user_id']))->getField('nickname');
                            if (in_array($v['user_id'],array(82))){
                                $url                = U('Inspect/satisfaction',array('uname'=>$uname,'kpiTime'=>$v['start_date'].','.$v['end_date'],'pin'=>2));
                            }else{
                                $url                = U('Inspect/satisfaction',array('uname'=>$uname,'kpiTime'=>$v['start_date'].','.$v['end_date'],'pin'=>1));
                            }
                        }
