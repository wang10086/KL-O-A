...
function get_sum_gross_profit($userids,$beginTime,$endTime){
    ...
    function get_user_contract_list($userid,$yearMonth,$begintime,$endtime){
        ...
        foreach ($op_list as $key=>$value){
            ...
            $time                           = $value['dep_time']; //出团前完成合同上传
            $list                           = M('contract')->where(array('op_id'=>$value['op_id'],'status'=>1))->find(); //查看截止当期有无合同

            if ($list){
                if ($list['confirm_time'] <= $time){
                    $contract_list[]               = $list;
...
//获取公司合计满意度
    ...
    function year_month_statis($year,$month,$account_ids,$type=0){
        ...
        $year_average                          = $year_shishi_num ? (round($year_op_average_sum/$year_shishi_num,4)*100).'%' : '100%'; //全部平均值,包括未调查的
        ...
        $month_average                          = $month_shishi_num ? (round($month_op_average_sum/$month_shishi_num,4)*100).'%' : '100%'; //全部平均值
...
//获取部门当月合计
 ...
 //获取公司合计满意度
    ...
    //城市合伙人满意度评分
    function get_partner_satisfaction($uid,$month,$quota_id=0){
        $month                      = explode(',',$month);
        ...
        foreach ($lists as $k=>$v){
            $lists[$k]['hide_mobile'] = hide_mobile($v['mobile']);
        ...
        $average                    = $score_sum ? round($score_num/$score_sum,2) : '0%';
        $data                       = array();
        $data['average']            = $average;
        $data['number']             = $number;
        $data['lists']              = $lists;
