    ...
    <div class="box-body">
        <div class="content">
            <div class="form-group col-md-12">
                ...
                <div class="form-group col-md-4">
                    <label>培训预算：{$need ? $need['price'] : $detail['price']}元/人</label>
                </div>

                <div class="form-group col-md-4">
                    <label>往返大交通：{$need ? $need['traffic'] : $detail['traffic']}</label>
                </div>

                <div class="form-group col-md-4">
                    <label>院所：
                        <?php echo $need ? ($need['is_need_ins']==1 ? '需要；' : '不需要；') : ($detail['is_need_ins']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['is_sure_ins']==1 ? '指定院所：' : '非指定院所；') : ($detail['is_sure_ins']==1 ? '指定院所：' : '非指定院所；'); ?>
                        <?php echo $need ? $need['ins_name'] : $detail['ins_name']; ?>
                    </label>
                </div>

                <div class="form-group col-md-4">
                    <label>企业：
                        <?php echo $need ? ($need['is_need_company']==1 ? '需要；' : '不需要；') : ($detail['is_need_company']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['is_sure_company']==1 ? '指定企业：' : '非指定企业；') : ($detail['is_sure_company']==1 ? '指定企业：' : '非指定企业；'); ?>
                        <?php echo $need ? $need['company_name'] : $detail['company_name']; ?>
                    </label>
                </div>

                <div class="form-group col-md-4">
                    <label>高校：
                        <?php echo $need ? ($need['is_need_school']==1 ? '需要；' : '不需要；') : ($detail['is_need_school']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['is_sure_school']==1 ? '指定高校：' : '非指定高校；') : ($detail['is_sure_school']==1 ? '指定高校：' : '非指定高校；'); ?>
                        <?php echo $need ? $need['school_name'] : $detail['school_name']; ?>
                    </label>
                </div>

                <div class="form-group col-md-4">
                    <label>景点：
                        <?php echo $need ? ($need['is_need_scenicSpot']==1 ? '需要；' : '不需要；') : ($detail['is_need_scenicSpot']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['is_sure_scenicSpot']==1 ? '指定景点：' : '非指定景点；') : ($detail['is_sure_scenicSpot']==1 ? '指定景点：' : '非指定景点；'); ?>
                        <?php echo $need ? $need['scenicSpot_name'] : $detail['scenicSpot_name']; ?>
                    </label>
                </div>

                <div class="form-group col-md-4">
                    <label>博物馆：
                        <?php echo $need ? ($need['is_need_museum']==1 ? '需要；' : '不需要；') : ($detail['is_need_museum']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['is_sure_museum']==1 ? '指定博物馆：' : '非指定博物馆；') : ($detail['is_sure_museum']==1 ? '指定博物馆：' : '非指定博物馆；'); ?>
                        <?php echo $need ? $need['museum_name'] : $detail['museum_name']; ?>
                    </label>
                </div>

                <div class="form-group col-md-4">
                    <label>开闭营：
                        <?php echo $need ? ($need['is_need_openingClosingCamp']==1 ? '需要' : '不需要') : ($detail['is_need_openingClosingCamp']==1 ? '需要' : '不需要'); ?>
                    </label>
                </div>
                ...
                <div class="form-group col-md-6">
                    <label>动手材料：
                        ...
                        <?php echo $need ? ($need['member'] ? '人数：'.$need['member'].'人；' : '') : ($detail['member'] ? '人数：'.$detail['member'].'人；' : ''); ?>
                        <?php echo $need ? ($need['num'] ? '次数：'.$need['num'] : '') : ($detail['num'] ? '次数：'.$detail['num'] : ''); ?>
                        <?php echo $need ? ($need['material_cost'] ? '费用预算：'.$need['material_cost'].'元' : '') : ($detail['material_cost'] ? '费用预算：'.$detail['material_cost'].'元' : ''); ?>
                    </label>
                </div>

                <div class="form-group col-md-12">
                    <label>是否需要领导出席及讲话：
                        <?php echo $need ? ($need['is_need_leader']==1 ? '需要；' : '不需要；') : ($detail['is_need_leader']==1 ? '需要；' : '不需要；'); ?>
                        <?php echo $need ? ($need['leader_time'] ? '出席时间：'.date('Y-m-d H:i:s',$need['leader_time']).'；' : '') : ($detail['leader_time'] ? '出席时间：'.date('Y-m-d H:i:s',$detail['leader_time']).'；' : ''); ?>
                        <?php echo $need ? ($need['leader_condition'] ? '要求：'.$need['leader_condition'] : '') : ($detail['leader_condition'] ? '要求：'.$detail['leader_condition'] : ''); ?>
                    </label>
                </div>

                <div class="form-group col-md-12">
                    <label>其他线路需求：{$need ? $need['other_line_condition'] : $detail['other_line_condition']}</label>
                </div>
                ...
                <div class="form-group col-md-12">
                    <label>住宿：
                        <?php echo $need ? ($need['star'] ? '星级：'.$need['star'].'星级；' : '') : ($detail['star'] ? '星级：'.$detail['star'].'星级；' : ''); ?>
                        <?php echo $need ? ($need['stay_price'] ? '预算价格：'.$need['stay_price'].'元；' : '') : ($detail['stay_price'] ? '预算价格：'.$detail['stay_price'].'元；' : ''); ?>
                        <?php echo $need ? ($need['house_num'] ? '房间数：'.$need['house_num'].'间；' : '') : ($detail['house_num'] ? '房间数：'.$detail['house_num'].'间；' : ''); ?>
                        <?php echo $need ? ($need['house_condition'] ? '要求：'.$need['house_condition'] : '') : ($detail['house_condition'] ? '要求：'.$detail['house_condition'] : ''); ?>
                    </label>
                </div>

                <div class="form-group col-md-12">
                    <label>餐食：早餐：{$need ? $need['breakfast_num'] : $detail['breakfast_num']} 餐；正餐：{$need ? $need['dinner_num'] : $detail['dinner_num']} 餐；餐标：{$need ? $need['dinner_price'] : $detail['dinner_price']} 元；要求：{$need ? $need['dinner_condition'] : $detail['dinner_condition']} ；
                        火车餐：
                        <?php echo $need ? ($need['is_need_train_meal'] ? '需要；数量'.$need['train_meal_num'].'餐；价格'.$need['train_meal_price'].'元；' : '不需要；') : ($detail['is_need_train_meal'] ? '需要；数量'.$detail['train_meal_num'].'餐；价格'.$detail['train_meal_price'].'元；' : '不需要；'); ?>
                    </label>
                </div>
