...
class SalaryController extends BaseController {
    ...
    /*public function salary_excel_list(){
        $monthly                            = I('month')?trim(I('month')):date('Ym');       //年月(搜索框)
        $archives                           = trim(I('archives'));                          //档案(科旅,中心)
        $datetime                           = trim(I('datetime'));
        $name                               = trim(I('name'));                              //名字

        $where                              = array();
        if ($archives) $where['a.archives'] = $archives;
        if ($name) $where['a.nickname']     = $name;
        if ($monthly) $where['s.datetime']  = $monthly;
        $where['s.status']                  = array('in',array(2,3));
        //$where['a.status']                = array('in',array(0,1));

        $wages_month                        = M()->table('__SALARY_WAGES_MONTH__ as s')->join('__ACCOUNT__ as a on a.id=s.account_id','left')->where($where)->select();
