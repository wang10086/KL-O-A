...
class AaatestController extends BasepubController {
...
//            $Year_end                               = $Year_end;//如果做季度提成可以变为奖金放开屏蔽即可
    ...
    public function test_salary(){
        ...
        $where                  = array();
        $where['datetime']      = array('in',$dateTime);
        $where['status']        = 4;
        //$where['account_id']    = 77;
        $salary_lists           = M('salary_wages_month')->where($where)->field($field)->order('id asc')->select();
        ...
        exportexcel($export_data,$title,'2019年1月至2020年6月公司全员工资表');
