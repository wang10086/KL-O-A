<?php
    ...
    class WagesController extends BaseController{
         ...
         *
         */
        public function salary_excel_listAAA(){//判断权限
        ...
        public function salary_excel_list(){
           ...
           /* if ($wagesLists){
                ...
                $wagesLists                     = $this-> get_wages_lists($accounts,$datetime);
           /* }*/
        ...
        public function get_wages_lists($accounts,$datetime){
            $mod                                = D('Wages');
            ...
            foreach ($accounts as $k=>$v){
                ...
                $data[$k]['standard_salary']    = $salary['standard_salary'];   //岗位薪酬
                $data[$k]['basic_salary']       = ($salary['standard_salary']/10)*$salary['basic_salary']; //基本工资
                $data[$k]['attendance']         = M('salary_attendance')->where(array('account_id'=>$v['id'],'status'=>1))->order('id desc')->getField('withdrawing');//员工考勤考核
                ...
                $data[$k]['quarter_royalty']    = $quarter_royalty_data['quarter_royalty']; //业务人员季度提成
         ...
         * 获取详情数据表
         */
        private function salary_excel_sqlAAA($archives='',$name='',$datetime=""){
            /*$pay_year                                   = (int)substr($datetime,0,4);
            $pay_month                                  = (int)substr($datetime,4);
            if (in_array($pay_month,array('01','04','07','10'))){   //季度后一个月发放该季度提成
                if ($pay_month==1) {
