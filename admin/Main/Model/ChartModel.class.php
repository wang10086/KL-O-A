        ...
        foreach($department as $key =>$val){ //查询数组中是否部门用户id
            ...
            foreach($kind as $ke =>$va){ //类型
                ...
                foreach($lists as $k => $v){//年度数据
                    if($v['kind']==$ke && in_array($v['create_user'],$account)){// && in_array($key,$departmentid)
